...
export const useConnectChatWebSocket = () => {
    //refs    
    const ws = inject('webSocketRef');
    const messages = inject('messages');
    const reconnectTokens = ref(3);
    ...
    const sendMessage = async (message) => {
        ...
        if (user) {
            ...
            try {
                const response = await axios.post(API_ACESS_ROUTE + `/api/message`, {
                    body: {
