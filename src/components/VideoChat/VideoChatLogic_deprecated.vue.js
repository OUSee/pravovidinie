...
const refVideo = inject('refVideo'); // refers to a video tag element 
const refUserVideo = inject('refUserVideo'); // refers to a video tag element 
const enableCall = inject('enableCall'); // hardcoded for mvp flag to start videochat
...
const notAnswered = ref(false);
const callError = ref(null);
const stopwatchSecondOffset = ref(new Date);
const guestBarEnabled = ref(false);
const callEnd = ref(false);
const isPlaying = ref(false);
const audioPlayer = ref(null);
const callingSound = ref(callSound);
...
const onClickClose = async (closeModal = false, reason = '') => {
...
const getMedia = async () => {
    ...
    openCamera().then((stream) => {
        ...
        webSocketRef.value = new WebSocket(`/join?roomID=${roomID.value}&token=${token}`);
        webSocketRef.value.onopen = () => {
            ...
            webSocketRef.value.addEventListener("message", async (e) => {
                ...
                if (message.answer) {
                    ...
                    webSocketRef.value.send(JSON.stringify({ type: "video_status", status: videoEnabled }));
                }
...
const handleVideoStream = () => {
    if (yourVideo.value) {
        ...
        yourVideo.value.getTracks().forEach((track) => {
...
const createVideoCallRoom = () => {
    ...
    return axios.post(`${BASE_API_URL}/create`, { profile: 'test' }, { headers: { Authorization: `Bearer ${token}` } });
};
const createRoom = () => {
    createVideoCallRoom()
        .then((response) => {
