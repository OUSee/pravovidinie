import { ref, watchEffect, inject, watch, onUnmounted } from 'vue';
...
export const useVideoChat = () => {
    ...
    const webSocketRef = inject('webSocketRef');
    // const yourVideo = ref<MediaStream | null>(null)
    const partnerStream = ref(null);
    ...
    const API_ACESS_ROUTE_WS = inject('API_ACESS_ROUTE_WS');
    const API_CREATE_ROOM = inject('API_CREATE_ROOM');
    ...
    const iceRestartTimer = ref(null);
    ...
    const handleNegotiationNeeded = () => {
        if (!peerRef || !peerRef.value || !webSocketRef || !webSocketRef.value)
            return;
        peerRef.value
            .createOffer()
            .then((offer) => peerRef.value?.setLocalDescription(offer))
            .then(() => {
            setTimeout(() => {
    ...
    const handleOffer = async (offer) => {
        ...
        await peerRef.value.setRemoteDescription(new RTCSessionDescription(offer)).catch(e => {
        ...
        webSocketRef && webSocketRef.value && webSocketRef.value.send(JSON.stringify({ answer: peerRef.value.localDescription }));
        stopSound();
    };
    const createPeer = () => {
            ...
            webSocketRef.value.onmessage = async (e) => {
                ...
                if (message.offer) {
                    ...
                    if (peerRef && peerRef.value) {
                        ...
                        webSocketRef && webSocketRef.value && webSocketRef.value.send(JSON.stringify({ type: "video_status", status: videoEnabled }));
    ...
    const createRoom = async () => {
        try {
            ...
            if (room && roomID) {
                ...
                const response = await axios.post(API_CREATE_ROOM + `/create`, { profile: 'test' }, { headers: { Authorization: `Bearer ${token}` } });
