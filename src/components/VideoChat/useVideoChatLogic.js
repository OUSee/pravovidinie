import { ref, watchEffect, inject, onUnmounted, nextTick, watch } from 'vue';
...
export const useVideoChatLogic = () => {
    ...
    const handleNegotiationNeeded = async () => {
        if (!peerRef.value || !webSocketRef?.value || isNegotiating.value)
            return;
    ...
    const handleOffer = async (offer) => {
        ...
        if (!userStream.value || !webSocketRef?.value)
            return;
        try {
    ...
    const getMedia = async () => {
        ...
        try {
            ...
            if (refUserVideo && refUserVideo.value) {
                ...
                await refUserVideo.value.play().catch((e) => console.warn('Ошибка воспроизведения локального видео:', e));
            }
            else {
